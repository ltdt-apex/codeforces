...
class Solution {
public:
    vector<int> maximumCount(vector<int>& v, vector<vector<int>>& qs) {
        int n = v.size();
        int m = qs.size();
        ...
        auto insert = [&](int i, int x){
            if(not isprime(x)) return;

            // cout << x << endl;
            
            if(d[x].empty()){
                st.range_add(0,n-1,1);
            }
            else if(i<*d[x].begin()){
                st.range_add(i,*d[x].begin()-1,1);
            }
            else if(i>*prev(d[x].end())){
                st.range_add(*prev(d[x].end()),i-1,1);
            }
        ...
        auto remove = [&](int i, int x){
            ...
            if(d[x].size()==1){
                ...
                st.range_add(i,*next(d[x].begin())-1,-1);
            }
            else if(i==*prev(d[x].end())){
                st.range_add(*prev(prev(d[x].end())),i-1,-1);
            }
