...
void test_normal_segment_tree(){
    ...
    assert(st.query(2,3) == 18);
    assert(st.query(4,4) == 10);
    assert(st.query(2,4) == 28);
    assert(st.query(0,4) == 27);
}

void test_min_segment_tree(){
    int n = 5;
    vi v(n);
    rep(i,0,n){
        v[i] = i;
...
void test_range_addition_segment_tree(){
    int n = 5;
    vi v(n);
    rep(i,0,n){
        v[i] = i;
    ...
    assert(st.query(1,1) == 8);
    assert(st.query(2,2) == 9);
    assert(st.query(3,3) == 3);
    assert(st.query(4,4) == 6);

    assert(st.query(0,2) == 22);
    assert(st.query(1,4) == 26);
    assert(st.query(2,3) == 12);
    assert(st.query(1,3) == 20);
}

void test_range_addition_max_segment_tree(){
    int n = 5;
    vi v(n);
    rep(i,0,n){
        v[i] = i;
    ...
    assert(st.query(3,4) == 6);
    assert(st.query(1,3) == 9);
}

void test_mixed_segment_tree(){
    int n = 5;
    vi v(n);
    rep(i,0,n){
        v[i] = i;
    ...
    assert(st.query(1,3) == 7);
}

void test() {
    assert(numeric_limits<int>::max() == INT_MAX);
    assert(numeric_limits<ll>::max() == LONG_LONG_MAX);
    assert(-numeric_limits<int>::max() == -INT_MAX);
    assert(-numeric_limits<ll>::max() == -LONG_LONG_MAX);

    test_normal_segment_tree();
    test_min_segment_tree();
    test_max_segment_tree();
    test_range_addition_segment_tree();
    test_range_addition_max_segment_tree();
    test_mixed_segment_tree(); // mixed operation actually won't work, the testcase is too weak to passed.
}
